...
from app.domain.ai.diagram.erd import DiagramErd


class DiagramErdAdapter(DiagramErdGeneratePort):
    ...
    async def generate(
        ...
        pInputs = {
            "keys": DiagramErd.generate_diagram_erd(
                query=DiagramErd.Query(query),
            ).model_dump()
        }
